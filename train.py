...
from schrodinger_bridge import compute_schrodinger_scores
...
def run_for_window(returns, window_days):
    ...
    try:
        raw_scores = compute_schrodinger_scores(
            ret_window,
    ...
    top_etfs = [{"ticker": t, "bridge_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    out_file = f"output/schrodinger_bridge_{timestamp}.json"
